...
def get_aperture_limits(tele,jwst_filepath):
    """Read in the aperture parameters relating to the noise in each image.

    Parameters:
    tele -- Str: Telescope of interest. Current options are 'jwst', 'hst', 'roman', 'subaru' and 'vista'
    ...
    return ap_f_limits, ap_sig, ap_radius
...
class Image():
    ...
    def get_image_params(self):
        ...
        if self.catalogue.z == 7:
            ...
            ff = fits.open(self.catalogue.image_path+'hlsp_bluetides_{}_{}_z{}_{}_v1_sim-psf.fits'.format(self.telescope, 
                           self.instrument, self.catalogue.z, self.filt))
    ...
    def add_noise(self, exp_time, ap_f_limit, ap_sig, ap_radius):
        ...
        for ii in range(0,self.catalogue.nGalsSelected):
            ...
            aperture_f_limit=ap_f_limit, aperture_significance=ap_sig, aperture_radius=ap_radius, verbose = False)
